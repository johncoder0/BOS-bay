...
/datum/map/proc/grid_check_announcement()
	command_announcement.Announce(replacetext(grid_check_message, "%STATION_NAME%", station_name()), "Автоматическая проверка энергосистем", new_sound = grid_check_sound) //cake

/datum/map/proc/grid_restored_announcement()
	command_announcement.Announce(replacetext(grid_restored_message, "%STATION_NAME%", station_name()), "Автоматическая проверка энергосистем", new_sound = grid_restored_sound) //cake
...
/datum/map/proc/level_x_biohazard_announcement(var/bio_level)
	...
	command_announcement.Announce("Подтверждена биологическая угроза [bio_level] уровня на [station_name()]. Всему персоналу требуется сдержать угрозу.", "Биоугроза", new_sound = level_x_biohazard_sound(bio_level)) //cake

/datum/map/proc/level_x_biohazard_sound(var/bio_level)
	return

/datum/map/proc/radiation_detected_announcement()
	command_announcement.Announce(replacetext(radiation_detected_message, "%STATION_NAME%", station_name()), "Аномальная угроза", new_sound = radiation_detected_sound) //cake

/datum/map/proc/space_time_anomaly_detected_annoncement()
	command_announcement.Announce("Пространственно-временные аномалии были обнаружены на [station_name()].", "Аномальная угроза", new_sound = space_time_anomaly_sound) //cake

/datum/map/proc/unidentified_lifesigns_announcement()
	command_announcement.Announce(replacetext(unidentified_lifesigns_message, "%STATION_NAME%", station_name()), "Неизвестная угроза", new_sound = unidentified_lifesigns_sound) //cake

/datum/map/proc/unknown_biological_entities_announcement()
	command_announcement.Announce(replacetext(unknown_biological_entities_message, "%STATION_NAME%", station_name()), "Неизвестная угроза", new_sound = command_report_sound) //cake
