...
/obj/item/clothing/head/helmet/refit_for_species(var/target_species)
	...
	switch(target_species)
		if(SPECIES_SKRELL)
			species_restricted = list(SPECIES_HUMAN,SPECIES_PLASMASANS,SPECIES_SKRELL,SPECIES_IPC) //skrell helmets fit humans too
		if(SPECIES_HUMAN)
	...
	if(href_list["list_ungabunga"])
		if(accessories.len)
			...
			for(var/accessory in accessories)
				ties += "[icon2html(src, viewers(src))]accessory] \a [accessory]"
